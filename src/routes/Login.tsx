...
const Login = () => {
    ...
    auth
      ...
      .then((code) => {
        switch (code) {
          ...
          case 400:
            setError(
              "Es wurde ein ungültiger Authentifizierungscode übergeben. Bitte versuchen Sie es später erneut.",
            );
            break;
          case 401:
            setError(
  ...
  return (
    <div className="flex min-h-dvh flex-col items-center justify-center py-24">
      ...
      {isLoading ? (
        <ProgressSVG width={60} height={60} className="shrink-0 animate-spin" />
      ) : (
        <div className="flex gap-6">
          ...
          <Button
            label={error ? "Erneut versuchen" : "Anmelden"}
