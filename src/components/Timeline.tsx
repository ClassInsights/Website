    ...
    {timelineData.map((item, index) => {
      ...
      return (
        <li
          ...
          className="relative flex items-baseline gap-6 pb-4"
        >
          <div
            className={
              isLast
                ? ""
                : "before:absolute before:left-[4px] before:h-full before:w-[2px] before:bg-black"
            }
          >
            <div
              className={`h-[0.625rem] w-[0.625rem] rounded-full bg-black ${isLast ? "bg-primary" : "bg-black"}`}
            />
          </div>
          <div>
