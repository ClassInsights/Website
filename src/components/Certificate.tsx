		...
		<dialog className="fixed top-0 z-20 flex h-dvh w-screen items-end justify-center bg-transparent md:items-center">
			...
			<div className="absolute h-[88%] w-full rounded-t-2xl bg-background p-4 md:h-auto md:w-10/12 md:rounded-2xl lg:w-4/6 xl:w-1/2 2xl:w-2/5">
				...
				<div className="scrollbar h-full overflow-y-scroll px-3 pt-12 pb-16 md:px-16 md:py-16">
					...
					<p className="mt-8">
						Das bedeutet, dass diese Schule eine innovative Softwarelösung zur Optimierung des Stromverbrauches
						verwendet &#x2015; <span className="text-primary">ClassInsights</span>.
					</p>
					<p className="my-6">
						Das Team von ClassInsights bedankt sich herzlich bei {certificate.certificateData?.name} für die tolle
						Zusammenarbeit.
					</p>
					...
					<div className="mt-14 flex w-full flex-col gap-8 md:flex-row md:gap-20">
						{["Jakob Wassertheurer", "Julian Grill"].map((name) => (
							<h2 key={name.split(" ")[0]} className="select-none font-light font-sacramento text-primary">
								{name}
							</h2>
						))}
					</div>
