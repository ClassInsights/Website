...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const isAuthenticated = useMemo(
    () => authData !== null && authData.expires_at > Date.now() / 1000,
    [authData],
  );
  const JWKS = useMemo(() => {
  ...
  const decodeAuthData = useCallback(
    async (data: AuthData | AuthResponse) => {
      ...
      const userData: UserData = {
        ...
        expires_at: isAuthData(data)
          ? data.expires_at
          : Math.floor(data.expires_in + Date.now() / 1000),
      };
  ...
  const startRefreshTimeout = useCallback(
    (expiresAt: number, refresh_token: string) => {
      ...
      refreshTimeout.current = window.setTimeout(async () => {
        ...
        startRefreshTimeout(
          Math.floor(response.expires_in + Date.now() / 1000),
  ...
  const exchangeCode = useCallback(
    async (code: string) => {
      const response = await fetch(`${conf().VITE_API_URL}/azure/login?code=${code}`);
